...
Все операции возвращают новые экземпляры структур, не изменяя исходные. Это достигается через:
- Использование `defrecord` для создания неизменяемых типов данных
- Рекурсивное построение новых структур вместо мутации существующих
- Отсутствие использования `atom`, `ref`, `agent` или других изменяемых ссылок

#### 2. Полиморфизм
Функции работают как с `SCBag`, так и с обычными коллекциями Clojure:
- Проверка типа через `instance?`
- Fallback на стандартные функции Clojure для обычных коллекций
- Единый интерфейс для работы с разными типами данных

#### 3. Эффективная функция сравнения
**`bag-equals?`** реализована эффективно:
- Сначала проверяет общее количество элементов и количество уникальных элементов
- Затем проверяет равенство счетчиков для каждого уникального элемента
- **Не использует** наивное приведение к спискам и их сортировку
- Временная сложность: O(min(n, m)), где n и m — количество уникальных элементов

#### 4. API не "протекает"
Все функции работают через публичный API:
- Внутреннее представление (`Node`, `SCBag`) скрыто от пользователя
- Все операции выполняются через публичные функции
- Тесты проверяют поведение через API, а не внутреннее представление
