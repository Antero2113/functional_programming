...
1. **Функциональный подход:** Все расчеты выполняются чистыми функциями без побочных эффектов

2. **Неизменяемость данных:** Все операции со состоянием создают новые структуры данных, не изменяя существующие

3. **Реактивность:** Использование подписок re-frame обеспечивает автоматическое обновление UI при изменении данных

4. **Модульность:** Логика расчета вынесена в отдельные модули (`calc/`), что упрощает тестирование и поддержку

5. **Разделение ответственности:** Четкое разделение на слои: представление (views), логика (events, subs), расчеты (calc)

6. **Hot reload:** Интеграция с shadow-cljs обеспечивает автоматическую перекомпиляцию и обновление приложения при изменении кода

7. **Работа с периодами:** Гибкая система периодов (месяцы, кварталы, полугодия, год) с автоматическим вычислением норм и агрегацией данных
